...
async def revoke_invite_handler(client, message: Message):
    ...
    Usage: /revote <channel_id> <invite_link>
    Revokes a Telegram invite link generated by the bot for a specific channel.
    """
    args = message.text.split(maxsplit=2)
    if len(args) != 3:
        await message.reply_text("Usage: /revote <channel_id> <invite_link>")
        return
    ...
    try:
        ...
        reply = await message.reply_text(f"✅ Invite link revoked for chat ID <code>{channel_id}</code>.", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        reply = await message.reply_text(f"❌ Failed to revoke invite link: {e}")
