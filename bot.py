...
async def revoke_invite_handler(client, message: Message):
    ...
    Usage: /revote <invite_link>
    Revokes a Telegram invite link generated by the bot.
    """
    args = message.text.split(maxsplit=1)
    if len(args) != 2:
        await message.reply_text("Usage: /revote <invite_link>")
        return
    invite_link = args[1].strip()
    # Try to find the channel/group this invite belongs to
    found = False
    result_msg = None

    # Loop through allowed channels to find which chat this invite belongs to
    for chan in allowed_channels_col.find({}, {"channel_id": 1}):
        chat_id = chan["channel_id"]
        try:
           # Try to revoke; if the link doesn't belong to this chat, pyrogram raises an error
           await client.revoke_chat_invite_link(chat_id, invite_link)
           result_msg = await message.reply_text(f"✅ Invite link revoked for chat ID <code>{chat_id}</code>.", parse_mode=enums.ParseMode.HTML)
           found = True
           break
        except Exception as e:
    ...
    if not found:
        result_msg = await message.reply_text("❌ Failed to revoke invite link. Make sure the link is valid and created by the bot for an allowed channel.")
    if result_msg:
        bot.loop.create_task(auto_delete_message(message, result_msg))
